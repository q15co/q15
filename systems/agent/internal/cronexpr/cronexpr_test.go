...
func TestParseAcceptsSupportedSyntax(t *testing.T) {
	t.Parallel()
...
func TestParseRejectsUnsupportedSyntax(t *testing.T) {
	...
	for _, tc := range cases {
		if _, err := Parse(tc); err == nil {
			t.Fatalf("Parse(%q) error = nil, want non-nil", tc)
		}
	}
}

func TestExpressionNextMatchesLeapYearAndDayRules(t *testing.T) {
	t.Parallel()
