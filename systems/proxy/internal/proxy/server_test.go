...
func TestStart_ProxiesHTTPRequests(t *testing.T) {
	...
	proxy, err := startTestProxy(ctx, t, Config{})
	if err != nil {
...
func TestStart_ReusesPersistedCACertAcrossRestarts(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	stateDir := t.TempDir()

	proxy, err := startTestProxy(ctx, t, Config{StateDir: stateDir})
	if err != nil {
		t.Fatalf("Start() error = %v", err)
	}
	caPath := proxy.CACertHostPath()
	...
	if err != nil {
		t.Fatalf("os.ReadFile(first CA) error = %v", err)
	}
	...
	defer cancel2()
	proxy2, err := startTestProxy(ctx2, t, Config{StateDir: stateDir})
	if err != nil {
		t.Fatalf("Start(restart) error = %v", err)
	}
	secondCA, err := os.ReadFile(proxy2.CACertHostPath())
	if err != nil {
		t.Fatalf("os.ReadFile(second CA) error = %v", err)
	}
	if string(firstCA) != string(secondCA) {
		t.Fatalf("expected CA PEM to persist across restarts")
	}
}
...
func TestStart_KeepsProxyReplacementStateIsolatedPerServer(t *testing.T) {
	...
	proxyA, err := startTestProxy(ctxA, t, Config{
	...
	proxyB, err := startTestProxy(ctxB, t, Config{
...
func startTestProxy(ctx context.Context, t *testing.T, cfg Config) (*Server, error) {
	t.Helper()
	if cfg.StateDir == "" {
